...
import { assertPinnedSource, computeSourceManifest, PINNED_SOURCE_MANIFEST_DIGEST, PINNED_SOURCE_MANIFEST_PATH, PINNED_SOURCE_PATHS, PINNED_UPSTREAM_REVISION, sourceManifestDigest } from './source-lock.js'
...
const manifest = computeSourceManifest(referenceRoot)
const manifestDigest = sourceManifestDigest(manifest)
if (manifestDigest !== PINNED_SOURCE_MANIFEST_DIGEST) {
  throw new Error(`Pinned source manifest digest mismatch: expected ${PINNED_SOURCE_MANIFEST_DIGEST}, received ${manifestDigest}. Regenerate the pin from the correct upstream revision (see source-lock.js).`)
}
...
const manifestPath = resolve(projectRoot, 'scripts', 'upstream', PINNED_SOURCE_MANIFEST_PATH)
await writeFile(manifestPath, JSON.stringify({ revision: upstreamRevision, paths: [...PINNED_SOURCE_PATHS], entries: manifest }, null, 1) + '\n')
