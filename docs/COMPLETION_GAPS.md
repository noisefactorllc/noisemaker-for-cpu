...
- Last verification: 2026-09-26 at `f5a431d`-tree (check commands executed on this exact tree state; the tree is identical to `ad00451` plus docs-only commits — `git diff ad00451..f5a431d --stat` shows only `docs/COMPLETION_GAPS.md`): `npm test`: 279 pass, 0 fail, 1 skipped with `NM_REFERENCE_ROOT` at upstream `6a0af04d` (277 pass / 2 skipped without it — the reference-tree manifest test at `test/upstream-source-lock.test.js:71` conditionally skips when the reference checkout is unavailable; the test set includes the isolated cos-wrap characterization test, executed as part of this run and whose wrapped-side metrics (94/105/5.86328125) were reproduced by an independent out-of-repo A/B probe on the same configuration). Full parity: 163/164 within ±2, 114 byte-exact, 41 skipped. Independent channel comparison (`compareRgba8` against the retained golden, re-executed): `filter/crt` max=80, mean=5.05859375, 89 channels over tolerance. Goldens and tolerances are unchanged.
