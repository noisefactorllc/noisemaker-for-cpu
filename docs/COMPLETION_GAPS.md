...
- Last verification: 2026-09-26 at `0d9dded` (all three required checks executed on that tree): `npm test`: 279 pass, 0 fail, 1 skipped with `NM_REFERENCE_ROOT` at upstream `6a0af04d` (277 pass / 2 skipped without it — the reference-tree manifest test at `test/upstream-source-lock.test.js:71` conditionally skips when the reference checkout is unavailable; the test set includes the wrap characterization test added in `bb1f4de`, executed as part of this run). Full parity: 163/164 within ±2, 114 byte-exact, 41 skipped. Independent channel comparison (`compareRgba8` against the retained golden, re-executed): `filter/crt` max=80, mean=5.05859375, 89 channels over tolerance. Goldens and tolerances are unchanged. The closure-record update that names `0d9dded` is docs-only; `npm test` was also re-executed on that update's own tree with identical results.
