...
- Evidence: `parity.log`, `probe.json`, and [CRT-PARITY.md](CRT-PARITY.md), all committed at the verification commit.
...
- Last verification: 2026-09-26 at `c2a1d18`. `npm test`: 278 pass, 0 fail, 1 skipped with `NM_REFERENCE_ROOT` at upstream `6a0af04d` (277 pass / 2 skipped without it — the reference-tree manifest test at `test/upstream-source-lock.test.js:71` conditionally skips when the reference checkout is unavailable; the test set is otherwise identical to the 278/1 record at `ba1c89a`, the only test-file change in between being a revision assertion in `test/upstream-inventory.test.js`). Full parity: 163/164 within ±2, 114 byte-exact, 41 skipped. Independent channel comparison (`compareRgba8` against the retained golden): `filter/crt` max=80, mean=5.05859375, 89 channels over tolerance. Goldens and tolerances are unchanged.
