...
- Last verification: 2026-09-26 at `2b8a6ec`-equivalent tree state (see commit list below; all three required checks executed on the candidate tree itself, including the test-file commit): `npm test`: 279 pass, 0 fail, 1 skipped with `NM_REFERENCE_ROOT` at upstream `6a0af04d` (277 pass / 2 skipped without it — the reference-tree manifest test at `test/upstream-source-lock.test.js:71` conditionally skips when the reference checkout is unavailable; the test set includes the isolated cos-wrap characterization test, executed as part of this run and whose wrapped-side metrics (94/105/5.86328125) were reproduced by an independent out-of-repo A/B probe on the same configuration). Full parity: 163/164 within ±2, 114 byte-exact, 41 skipped. Independent channel comparison (`compareRgba8` against the retained golden, re-executed): `filter/crt` max=80, mean=5.05859375, 89 channels over tolerance. Goldens and tolerances are unchanged.
