...
| 2026-09-26 | `b644b468` (merge of verified sync `bfbe5476` through upstream `8eeb7b5ac14e`) | Register pass-history row added; tree otherwise unchanged from the integrated remote sync. The tree pins upstream `8eeb7b5ac14e` with manifest digest `7382c8ccee81a540d48e1298911d6960de9b2970817a0709ef1da58ab99567a8`. Earlier in this job, candidate `41b92689` had regenerated kernel artifacts with per-record `sourceSha256` and added the checkout-free manifest/coverage tie-in test while pinning upstream `9d3474dfdc6c`; the later verified remote sync `bfbe5476` superseded that pin state. No effect behavior, tolerances, or goldens changed in the register row | At merge `b644b468`: `npm test` (node --test) 277 pass / 0 fail / 2 environment-gated skips; `npm run parity` exit 0 (GPU-golden effects skipped by design); `node --test test/upstream-source-lock.test.js` passes, including the manifest-anchor and coverage/manifest tie-in assertions against the committed artifacts. Exact-source CI at `41b92689`: export-kit run 36204154657 and downstream run 36204154669 both succeeded (delivery dispatches, not test runs). The repo's own unit suite was executed locally; the repo has no workflow that runs it | No gap closures claimed. GAP-001 CRT failure and the other seven gaps remain as recorded. Full rendered parity remains unverified |
