...
Of 304 canonical programs, 294 are generated and 10 use full CPU adapters: 4 fragment-kernel replacements (`classicNoisedeck/fractal`, `filter/historicPalette`, `filter/palette`, `synth/julia`) that directly translate the pinned shader behavior where an explicit CPU implementation is clearer or faster, plus 5 vertex-stage scatter adapters (`filter3d/flow3d`, `points/dla`, `points/lenia`, `points/physarum`, `render/pointsBillboardRender`, `render/pointsRender` — the last two share one `deposit` scatter key with viewMode/blendMode-selected variants, see below) for passes that rasterize a variable point or quad count rather than filling every destination pixel exactly once, plus `synth/remap` (new): `glsl-transpiler` mishandles its reference `struct ZoneTest { bool inside; float d2; }` — it generates code that treats each struct FIELD as a 2-component vector, crashing at runtime — so it gets a hand-written CPU adapter (`src/effects/adapters/remap.js`) that ports the same zone-routing algorithm directly, bypassing the struct entirely. The fractal adapter preserves the canonical Julia, Newton, Mandelbrot, and color-mapping paths; it is not a substitute algorithm. Additional narrowly scoped scalar adapters may replace a generated factory at dispatch time while retaining the generated kernel as a fallback; that does not change the 294/10 coverage split because both entries exist for those keys.

Reference `0ed489ec4684`'s `pointsRender`/`pointsBillboardRender` perspective view mode is the first upstream pattern to combine a pass-level `defines` override with `#if`/`const int X = MACRO`-style compile-time identifiers on a *non-scatter* (regular generated) program — `depthKeys`, `depthMerge`, `spriteMean`, `spriteMeanTiles`, `clearDefocus`. Unlike the reference (which bakes a separate shader per define combination), this port declares each such macro name as an ordinary `uniform` (see `compile-glsl.js`'s `runtimeDefines()`, which now scans pass-level `defines` in addition to `define`-typed globals) and binds its literal value at call time (`renderer.js`'s `passUniforms()`). This works because `glsl-transpiler` doesn't eliminate an unresolved `#if MACRO == N` at transpile time — it carries it through as an ordinary runtime `if` — so one generated kernel correctly serves every viewMode/blendMode variant of a program, with no per-combination factory needed.
