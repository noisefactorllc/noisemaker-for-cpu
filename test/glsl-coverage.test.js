...
test('every eligible canonical GLSL program is transpiled or assigned a parity adapter', () => {
  assert.equal(programCoverage.length, 214)
  assert.equal(programCoverage.filter((program) => program.status === 'generated').length, 210)
  ...
  assert.deepEqual([...new Set(programCoverage.map((program) => program.effectId))].length, 169)
